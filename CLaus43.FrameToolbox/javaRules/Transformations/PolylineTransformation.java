...
public class PolylineTransformation {
	...
	public static void translateToPoint(PolyLine polyline, Point centerPoint) {
		...
		// get all points
		List<Point> points = polyline.getElement();
		if (points.isEmpty()) {
			return;
		}
	...
	/**
	 * Rotates a PolyLine into a new Coordinate System with the new Z-Direction newZAxis. 
	 * The Orientation is calculated in a certain way, so that the Y-Axis stays in the XY-Plane.
	 * This can be fixed by a Rotation along the newZAxis, which is done by another function
	 * (This method of calculation should be quite cheap).
	 * 
	 * @param PolyLine polyline
	 * @param Direction newZAxis
	 */
	...
	/**
	 * Rotate a PolyLine along a Direction. 
	 * (The Rotation is done by a Quaternion Rotation)
	 * 
	 * @param PolyLine polyline
	 * @param Direction rotAxis
	 * @param Double angle
	 */
